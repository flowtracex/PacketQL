...
func (n *Normalizer) Normalize(zeekLog *ZeekLog) (*NormalizedEvent, error) {
	...
	for zeekField, normField := range rule.Promote {
		...
		switch normField {
		case "event_time":
			// Accept Zeek ts as numeric seconds, numeric string, or RFC3339 string.
			if tsMs, ok := parseZeekTimestampMs(value); ok {
				event.EventTime = tsMs
			}
		case "src_ip":
...
func parseZeekTimestampMs(v interface{}) (int64, bool) {
	switch val := v.(type) {
	case float64:
	...
	case string:
		...
		// Numeric string (Zeek epoch seconds, optionally decimal).
