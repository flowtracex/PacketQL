...
// Registry keys are all-uppercase with underscores: "dns" → "DNS", "smb_files" → "SMB_FILES"
func (pw *ParquetWriter) logTypeToStructName(logType string) string {
	return strings.ToUpper(logType)
}
...
func (pw *ParquetWriter) getStructTypeByName(structName string) reflect.Type {
	...
	schemaTypes := map[string]reflect.Type{
		...
		"FILES":       reflect.TypeOf(schema.FILES{}),
		"WEIRD":       reflect.TypeOf(schema.WEIRD{}),
		"DPD":         reflect.TypeOf(schema.DPD{}),
		"NOTICE":      reflect.TypeOf(schema.NOTICE{}),
	}
